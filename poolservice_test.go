...
func TestNewDefaultSizePool(t *testing.T) {
	...
	assert.NotNil(t, testPoolService, "pool1 service should not be nil")
	assert.Equal(t, 10, testPoolService.workerPool.poolCapacity, "default pool1 size should be 10")
...
func TestNewFixedSizePool(t *testing.T) {
	...
	assert.NotNil(t, testPoolService, "pool1 service should not be nil")
	assert.Equal(t, 15, testPoolService.poolSize, "pool1 size should be 15")
...
func TestNewFixedSizePoolWhenPoolSizeIsNegative(t *testing.T) {
	...
	defer func() {
		if r := recover(); r != nil {
			assert.Equal(t, "Invalid pool1 size: pool1 size must be a positive number", r.(error).Error())
		}
	}()
...
func TestPoolService_Submit_WhenPoolIsClosed(t *testing.T) {
	...
	assert.Equal(t, "Pool is closed: cannot assign task to a closed pool1", err.Error())
